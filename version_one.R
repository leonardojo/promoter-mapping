...
ggplot(data = table) + 
  ...
  geom_point(data = subset(table, (V6 == "-") & (V12 == "+")), aes(x=motif.middle, y= -0.15, fill = V10), shape = 24, size = 3, show.legend = FALSE) +
  geom_point(data = subset(table, (V6 == "-") & (V12 == "-")), aes(x=motif.middle, y= 0.15, fill = V10), shape = 25, size = 3) +
  geom_point(aes(x=450,y=0.1, fill = "TSS", shape = "TSS"),shape = 42, size = 6, show.legend = F) +
  geom_point(data = point, aes(x= X, y =0.01), fill= "grey20", shape = 108, size = 3) +
  xlim(1,500) + 
  ylim(-0.5,0.5) +
  theme_bw() +
  ...
  scale_shape_manual(values=c(25,25,25,42)) +
  theme(legend.position = "top", axis.title.y=element_blank(), axis.title.x = element_blank(), axis.text.y=element_blank(),axis.ticks.y=element_blank(),axis.text.x=element_blank(),axis.ticks.x=element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
